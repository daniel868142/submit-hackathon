...
class AnnotationSolution(HackathonSolution):
    ...
    maturity_description: str = "maturity date is a date which the swap ends, if no specific date in day, month, year is given then make sure maturity date is set to none. maturity tenor is the swap length or tenor years and is given as a year. look for year and give value as a year for tenor year."
    """Description of the maturity tenor or date to use with the parameter annotation prompt."""

    effective_date_description: str = "Effective date or start date, if none put default value as none"
    ...
    fixed_rate_description: str = "Fixed rate value, can be given as a percentage coupon from client, or given as the first currency within a swap's amount in percentage. For example, if EURUSD swap and client pays 6%, the value I want is 6."
    """Description of the fixed rate value to use with the parameter annotation prompt."""

    notional_description: str = "if the currencies are the same then notional amount is for both pay and receive. if its a currency swap then notional amount is only set for the leg where the currency is the same, set the notional amount of the other leg to none. for example if receiving japanese yen, and notional amount is in japanese yen then only set notional amount for receiving."
